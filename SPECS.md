...
- The logo text has a pronounced, wide canvas-colored outer glow that extends visibly beyond the immediate glyph edges, separates it from overlapping canvas content, and makes it read as floating above that content.
...
- Add node displays Double Click as its shortcut indicator and has no single-key keyboard shortcut.
